...
2. Determine whether lint and tests need to run:
   - Both `npm run lint` and `npm test` only target `src/`, so they are required only when changes include files under `src/`, or config files that affect them (`eslint.config.mjs`, `jest.config.js`, `babel.config.js`, `tsconfig*.json`, `package.json`, `package-lock.json`).
   - If no such files changed (e.g. only docs, `.claude/`, GitHub workflows, public assets), skip steps 3 and 4.
3. Run `npm run lint:fix` to auto-fix lint issues. If lint errors remain after the fix, report them and stop.
4. Run `npm test`. If tests fail, report the failures and stop.
